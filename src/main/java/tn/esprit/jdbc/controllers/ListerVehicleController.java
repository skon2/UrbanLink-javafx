...
public class ListerVehicleController {
    ...
    @FXML
    private TextField searchField; // Make sure this is properly defined
    ...
    private void filterVehicles(String query) {
        try {
            List<Vehicle> vehicles = vehicleService.showAll(); // Get all vehicles
            ObservableList<Vehicle> filteredList = FXCollections.observableArrayList();

            for (Vehicle v : vehicles) {
                if (v.getModel().toLowerCase().contains(query.toLowerCase()) ||
                        v.getLicensePlate().toLowerCase().contains(query.toLowerCase()) ||
                        v.getType().name().toLowerCase().contains(query.toLowerCase())){
                    filteredList.add(v);
                }
            }
    ...
    private void openAddVehicleForm() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/AjouterVehicle.fxml"));
